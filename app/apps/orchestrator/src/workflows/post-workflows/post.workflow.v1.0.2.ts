import type { PostActivity } from '../../activities/post.activity';
import {
  ActivityFailure,
  ApplicationFailure,
  startChild,
...
import { makeId, postIdSearchParam } from '../workflow.utils';
...
export async function postWorkflowV102({
  ...
  if (post.integration?.refreshNeeded) {
    ...
    await sendOrgAlert(
      ...
      `*${post.integration?.name}* needs to be reconnected. Go to Settings → Integrations to re-authenticate.`
    );
    return;
  }
  ...
  if (post.integration?.disabled) {
    ...
    await sendOrgAlert(
      ...
      'disconnected',
      post.integration?.providerIdentifier ?? 'unknown',
      `*${post.integration?.name}* is disabled. Go to Settings → Integrations to re-enable it.`
    );
    return;
  }

  const complianceGate = await gatePublishByComplianceOrHold(
    organizationId,
    post.group
  );
  if (!complianceGate.ok) {
    await inAppNotification(
      post.organizationId,
      'Post held for compliance',
      'Your scheduled post was not published because it is not cleared by Content Cop yet. Open the post to review, fix content if needed, or wait for approval.',
      true,
      false,
      'warning'
    );
    await sendOrgAlert(
      ...
      'held',
      post.integration?.providerIdentifier ?? 'unknown',
      `A post for *${post.integration?.name}* was held — Content Cop review pending. <${process.env.FRONTEND_URL || 'http://localhost:4200'}/launches|Fix or wait for approval>`
    );
    return;
  }
      ...
      try {
        ...
        if (
          ...
          await sendOrgAlert(
            ...
            'error',
            post.integration?.providerIdentifier ?? 'unknown',
            `Failed to publish for *${post.integration?.name}*${err?.cause?.message ? `: ${err.cause.message}` : ''}`
          );
          return false;
        }
      }
