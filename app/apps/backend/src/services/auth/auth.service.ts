...
export class AuthService {
  ...
  async checkExists(
    ...
    userAgent: string,
    datafast_visitor_id?: string
  ): Promise<{ jwt: string; isNewRegistration: boolean }> {
    const providerInstance = this._providerManager.getProvider(provider);

    let token: string;
    try {
      token = await providerInstance.getToken(code, redirectUri);
    } catch (e: unknown) {
      const raw = e as Record<string, unknown>;
      const apiError =
        (raw?.response as Record<string, unknown>)?.data ??
        (raw?.response as Record<string, unknown>)?.body;
      const errCode =
        (apiError as Record<string, string> | undefined)?.error ?? '';
      const errDesc =
        (apiError as Record<string, string> | undefined)?.error_description ??
        (e instanceof Error ? e.message : String(e));
      if (errCode === 'invalid_grant') {
        throw new BadRequestException(
          'The sign-in code has expired or was already used. Please sign in again.'
        );
      }
      if (errCode === 'redirect_uri_mismatch') {
        throw new BadRequestException(
          'OAuth redirect URI mismatch. Contact support.'
        );
      }
      throw new BadRequestException(
        errDesc || 'Failed to exchange OAuth code. Please try signing in again.'
      );
    }

    const oauthUser = await providerInstance.getUser(token);
    if (!oauthUser) {
      throw new BadRequestException('Could not retrieve user profile from provider.');
    }
    ...
    if (existing) {
      return { jwt: await this.jwt(existing), isNewRegistration: false };
    }
    ...
    const createdUser = await this.loginOrRegisterProvider(
      provider as Provider,
      {
        providerToken: token,
    ...
    return { jwt: await this.jwt(createdUser), isNewRegistration: true };
  }
