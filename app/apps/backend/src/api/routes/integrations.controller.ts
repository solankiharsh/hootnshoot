...
export class IntegrationsController {
  ...
  async getIntegrationList(@GetOrgFromRequest() org: Organization) {
    const allIntegrations = await this._integrationService.getIntegrationsList(org.id);
    const mapped = await Promise.all(
      allIntegrations.map(async (p) => {
        const findIntegration = this._integrationManager.getSocialIntegration(
          p.providerIdentifier
        );
        if (!findIntegration) {
          console.warn(`[integrations/list] unknown provider "${p.providerIdentifier}" for integration ${p.id} — skipping`);
          return null;
        }
        return {
          name: p.name,
    ...
    return { integrations: mapped.filter(Boolean) };
  }
  ...
  async getIntegrationUrl(
    ...
    try {
      ...
      console.error('[Integration] generateAuthUrl failed for', integration, ':', err);
      return { err: true };
    }
  }
