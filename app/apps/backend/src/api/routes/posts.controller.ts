...
export class PostsController {
  ...
  async createPost(
    ...
    if (body.type !== 'draft') {
      const publishWhenApproved = body.publishWhenApproved === true;
      const groups = [
        ...new Set(
          (result as { group?: string }[])
            .map((r) => r.group)
            .filter((g): g is string => typeof g === 'string' && g.length > 0)
        ),
      ];
      for (const group of groups) {
        void this._compliance
          .queueComplianceCheckAfterSave(org.id, group, { publishWhenApproved })
          .catch((err: unknown) => {
            this._logger.warn(
              `queueComplianceCheckAfterSave failed: ${
