...
export class PublicController {
  ...
  @Post('content-cop/callback')
  @UseGuards(ComplianceFeatureGuard)
  @HttpCode(200)
  contentCopWebhook(@Body() body: unknown) {
    void this._complianceService.handleWebhook(body).catch((err: unknown) => {
      this.logger.error('ContentCop webhook async error', err as Error);
    });
  ...
  @Get('/feature-flags')
  featureFlags() {
    return { complianceEnabled: isComplianceEnabled() };
  }
}
