...
export class UsersController {
  ...
  async setupWorkspace(
    @GetUserFromRequest() user: User,
    @Body() body: { mode: 'personal' | 'organization'; company?: string },
    @Res({ passthrough: true }) response: Response
  ) {
    let newOrg: { id: string };
    if (body.mode === 'personal') {
      newOrg = await this._orgService.createPersonalOrg(
        user.id,
        // @ts-ignore
      ...
      if (!body.company || body.company.trim().length < 2) {
        throw new HttpException('Company name required', 400);
      }
      newOrg = await this._orgService.createNamedOrg(user.id, body.company);
    }
    response.cookie('showorg', newOrg.id, {
      domain: getCookieUrlFromDomain(process.env.FRONTEND_URL!),
      ...(!process.env.NOT_SECURED
        ? {
            secure: true,
    ...
    if (process.env.NOT_SECURED) {
      response.header('showorg', newOrg.id);
    }
