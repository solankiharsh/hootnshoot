...
export class AuthController {
  ...
  mobileCallback(
    ...
    const scheme = process.env.MOBILE_APP_SCHEME || 'hootnshoot://auth/callback';
  ...
  async oauthLink(
    ...
    const oidcState =
      upper === 'GENERIC' ? randomBytes(24).toString('base64url') : undefined;
    const link = await this._authService.oauthLink(
      provider,
      oidcState ? { ...query, state: oidcState } : query
    );
    if (upper === 'GENERIC' && oidcState) {
      response.cookie('hootnshoot_oidc_state', oidcState, {
        domain: getCookieUrlFromDomain(process.env.FRONTEND_URL!),
        maxAge: 600_000,
        httpOnly: true,
        path: '/',
    ...
    return response.type('text/plain').send(link);
  }
  ...
  async oauthExists(
    ...
    @Body('datafast_visitor_id') datafast_visitor_id: string | undefined,
    @Param('provider') provider: string,
    @Res({ passthrough: false }) response: Response,
    @RealIP() ip: string,
    @UserAgent() userAgent: string
    ...
    if (provider.toUpperCase() === 'GENERIC') {
      const expected = req.cookies?.hootnshoot_oidc_state;
      if (!expected || !state || expected !== state) {
        response.clearCookie('hootnshoot_oidc_state', oidcCookieOpts);
        return response
          .status(400)
          .send(
            'Invalid or missing OAuth state. Close other login tabs and start sign-in again.'
          );
      }
      response.clearCookie('hootnshoot_oidc_state', oidcCookieOpts);
    }

    const { jwt, isNewRegistration } = await this._authService.checkExists(
      provider,
      code,
      redirect_uri,
      ip,
