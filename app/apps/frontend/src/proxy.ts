...
export async function proxy(request: NextRequest) {
  ...
  if (
    ...
    if (nextUrl.pathname === '/') {
      // Authenticated users belong in the app, not the marketing page
      if (authCookie) {
      ...
      // Unauthenticated OAuth callback — forward to /auth so Register component processes it
      if (
        nextUrl.searchParams.get('code') &&
        nextUrl.searchParams.get('provider')
      ) {
        return NextResponse.redirect(
          new URL(`/auth${new URL(nextUrl).search}`, nextUrl.href)
        );
      }
    }
    return topResponse;
    ...
    const additional = !findIndex
      ...
      : (url.indexOf('?') > -1 ? '&' : '?') +
        `provider=${(findIndex === 'settings'
          ? process.env.HOOTNSHOOT_GENERIC_OAUTH
            ? 'generic'
            : 'github'
          : findIndex
        ).toUpperCase()}`;
  ...
  // Strip OAuth callback params (code, provider, state, etc.) to avoid re-triggering the OAuth redirect guard
  if (nextUrl.pathname.startsWith('/auth') && authCookie) {
    const cleanParams = new URLSearchParams(url);
    ['code', 'provider', 'state', 'iss', 'scope', 'authuser', 'hd', 'prompt'].forEach(
      (p) => cleanParams.delete(p)
    );
    const cleanQuery = cleanParams.toString();
    return NextResponse.redirect(
      new URL(`/${cleanQuery ? '?' + cleanQuery : ''}`, nextUrl.href)
    );
  }
  if (nextUrl.pathname.startsWith('/auth') && !authCookie) {
    if (org) {
