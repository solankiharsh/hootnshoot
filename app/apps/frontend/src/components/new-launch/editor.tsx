...
export const Editor: FC<{
  ...
  const insertGeneratedCaption = useCallback(
    (payload: { headline: string; body: string; hashtags: string }) => {
      const ed = editorRef?.current?.editor;
      if (!ed) {
        return;
      }
      const html = captionResultToHtml(payload);
      if (ed.isEmpty) {
        ed.chain().focus().insertContent(html).run();
      } else {
        ed.chain().focus('end').insertContent(`<p><br></p>${html}`).run();
      }
    },
    []
  );
              ...
              {setImages && (
                <MultiMediaComponent
                  ...
                  captionGenerateAction={
                    editorType !== 'none' ? (
                      <CaptionGenerateControl
                        onCaptionReady={insertGeneratedCaption}
                      />
                    ) : undefined
                  }
                  information={
