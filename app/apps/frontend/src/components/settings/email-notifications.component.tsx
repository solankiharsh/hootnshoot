...
const EmailNotificationsComponent = () => {
  ...
  const [localSettings, setLocalSettings] = useState<EmailNotifications>({
    ...
    sendComplianceDigestEmails: false,
  });
  ...
  const handleComplianceDigestChange = useCallback(
    (value: 'on' | 'off') => {
      updateSetting('sendComplianceDigestEmails', value === 'on');
    },
    [updateSetting]
  );
    ...
    <div className="my-[16px] mt-[16px] bg-sixth border-fifth border rounded-[4px] p-[24px] flex flex-col gap-[24px]">
      ...
      {complianceEnabled && (
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <div className="text-[14px]">
              {t('compliance_digest_emails', 'Compliance Digest Emails')}
            </div>
            <div className="text-[12px] text-customColor18">
              {t(
                'compliance_digest_emails_description',
                'Weekly summary of posts approved, flagged, and overridden by Content Cop'
              )}
            </div>
          </div>
          <Slider
            value={localSettings.sendComplianceDigestEmails ? 'on' : 'off'}
            onChange={handleComplianceDigestChange}
            fill={true}
          />
        </div>
      )}
