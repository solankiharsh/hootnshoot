...
export const ContinueIntegration: FC<{
  ...
  const modifiedParams = useMemo(() => {
    ...
    // Late API providers send profileId + accountId instead of state + code
    if (provider.endsWith('-late')) {
      return {
        state: searchParams.profileId || searchParams.state || '',
        code: searchParams.accountId || searchParams.code || '',
      };
    }
  ...
  }, [modifiedParams, provider, fetch, navigateOrShow, logged]);
