...
import {
  providerIdentifierToContentCopPlatform,
  isContentCopProviderSupported,
  type ContentCopPlatform,
} from '@gitroom/frontend/components/compliance/content-cop-platform';
import { LaunchVariantsModal } from '@gitroom/frontend/components/launches/launch-variants.modal';
...
const CONTENT_COP_GATE_PLATFORMS: ReadonlySet<string> = new Set([
  'Instagram',
  'Facebook',
  'X',
  'TikTok',
...
function complianceGatePlatform(
  job: CalendarComplianceJobRow | null,
  providerId: string
): ContentCopPlatform {
  const p = job?.platform;
  if (p && CONTENT_COP_GATE_PLATFORMS.has(p)) {
    return p as ContentCopPlatform;
  }
  return providerIdentifierToContentCopPlatform(providerId) || 'Instagram';
}

function calendarComplianceBadge(job: CalendarComplianceJobRow | null | undefined): {
  variant: 'none' | 'pill' | 'dot';
  label: string | null;
  pillClass: string;
  clickable: boolean;
} {
  if (!job) {
    return { variant: 'none', label: null, pillClass: '', clickable: false };
  }
  const { status, decision } = job;
  if (status === 'pending' || status === 'running') {
    return {
      variant: 'pill',
      label: 'In Review',
      pillClass: 'bg-[#6b7280] text-white',
      clickable: true,
    };
  }
  ...
  if (status === 'completed' && decision === 'Needs Compliance Review') {
    return {
      variant: 'pill',
      label: 'Under review',
...
const usePostActions = (onMutate?: () => void) => {
  ...
  const editPost = useCallback(
    (
      loadPost: any,
      isDuplicate?: boolean,
      options?: { complianceFixMode?: boolean }
    ) => async () => {
      const post = {
        ...loadPost,
      ...
      const complianceJob =
        data.complianceJob ?? post.complianceJob ?? null;
      ...
      modal.openModal({
        ...
        children: (
          <ExistingData value={{ ...data, complianceJob }}>
            <AddEditModal
              complianceFixMode={options?.complianceFixMode}
              {...(isDuplicate
                ? {
  ...
  const openLaunchVariants = useCallback(
    (post: any) => () => {
      modal.openModal({
        title: t('launch_variants_title', 'Launch Language Variants'),
        closeOnClickOutside: true,
        closeOnEscape: true,
        withCloseButton: true,
        classNames: { modal: 'w-[100%] max-w-[560px] text-textColor' },
        children: (close: () => void) => (
          <LaunchVariantsModal
  ...
  return { editPost, deletePost, copyDebugJson, openStatistics, openMissingRelease, openLaunchVariants };
};
...
export const ListView = () => {
  ...
  const { editPost, deletePost, copyDebugJson, openStatistics, openMissingRelease, openLaunchVariants } = usePostActions();
            ...
            <div className="flex flex-col gap-[10px] mb-[20px] px-[10px]">
              {datePosts.map((post) => (
                <CalendarItem
                  ...
                  fixPost={editPost(post, false, { complianceFixMode: true })}
                  duplicatePost={editPost(post, true)}
                  copyDebugJson={user?.isSuperAdmin ? copyDebugJson(post) : undefined}
                  post={post}
                  integrations={integrations}
                  deletePost={deletePost(post)}
                  launchVariants={openLaunchVariants(post)}
                  showTime={true}
                />
              ))}
            </div>
...
const CalendarItem: FC<{
  ...
  post: Post & {
    ...
    complianceJob?: CalendarComplianceJobRow | null;
  };
}> = memo((props) => {
  const t = useT();
  const {
  ...
  const copSupported = isContentCopProviderSupported(providerId);
  const complianceJob = post.complianceJob ?? null;
  const baseComplianceBadge = calendarComplianceBadge(complianceJob);
  const complianceBadge =
    baseComplianceBadge.variant === 'none' && state === State.QUEUE
      ? {
          variant: 'pill' as const,
          label: copSupported
            ? t('compliance_in_review', 'In Review')
            : t('compliance_no_scan', 'No compliance scan'),
          pillClass: copSupported
            ? 'bg-[#6b7280] text-white'
            : 'bg-[#4b5563] text-white',
          clickable: copSupported,
        }
      : baseComplianceBadge;
  ...
  return (
    <div
      ...
      className={clsx(
        ...
        'relative',
        state === State.ERROR && 'rounded-[10px] ring-2 ring-red-500',
        state === State.HELD_COMPLIANCE &&
    ...
    >
      ...
      >
        ...
        {launchVariants && post.complianceJob?.decision === 'Approved' && (
          <div
            className={clsx(
              'hidden group-hover:block hover:underline cursor-pointer',
              post?.tags?.[0]?.tag?.color && 'mix-blend-difference'
            )}
            onClick={(e) => { e.stopPropagation(); launchVariants(); }}
          >
            <GlobeIcon />
          </div>
        )}
      ...
      >
        ...
        {complianceBadge.variant === 'dot' ? (
          <span
            className="absolute bottom-[8px] start-[8px] z-[15] h-[8px] w-[8px] rounded-full bg-green-500 shadow-sm"
            title={t('compliance_approved', 'Compliance approved')}
            data-tooltip-id="tooltip"
          ...
          complianceBadge.clickable ? (
            <button
              type="button"
              className={clsx(
                'absolute bottom-[6px] start-[6px] z-[15] max-w-[calc(100%-12px)] truncate text-[10px] font-[600] px-[6px] py-[2px] rounded-full shadow-sm text-start',
                complianceBadge.pillClass,
                'cursor-pointer hover:opacity-90'
              )}
              onClick={(e) => {
                e.stopPropagation();
                modal.openModal({
                  id: `compliance-gate-${post.group}-${post.id}`,
                  title: t('compliance_check_title', 'Compliance check'),
                  classNames: { modal: 'max-w-[640px] text-textColor' },
                  children: (close) => (
                    <div className="p-[8px]">
                      <ComplianceGatePanel
                        postGroupId={post.group}
                        platform={complianceGatePlatform(
                          complianceJob,
                          providerId
                        )}
                        initialJob={
                          complianceJob
                            ? (complianceJob as ComplianceJobDto)
                            : undefined
                        }
                        onClose={close}
                        onApproved={() => {
            ...
            <div
              className={clsx(
                'absolute bottom-[6px] start-[6px] z-[15] max-w-[calc(100%-12px)] truncate text-[10px] font-[600] px-[6px] py-[2px] rounded-full shadow-sm text-start pointer-events-none',
                complianceBadge.pillClass
              )}
            >
              {complianceBadge.label}
...
const GlobeIcon = () => {
  ...
  return (
    <svg
      ...
      strokeLinejoin="round"
      data-tooltip-id="tooltip"
      data-tooltip-content={t('launch_variants', 'Launch Language Variants')}
    >
