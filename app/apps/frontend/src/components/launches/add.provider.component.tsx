    ...
    <div className="flex flex-col gap-[16px] pt-[8px]">
      <p className="text-[14px] text-textColor/80">
        {t(
          ...
          'The Hootnshoot browser extension is not installed. You need to install it before connecting this channel.'
        )}
      </p>
      <div className="flex gap-[10px]">
        <Button
          ...
          onClick={() => {
            window.open(
              'https://chromewebstore.google.com/detail/hootnshoot/cidhffagahknaeodkplfbcpfeielnkjl?hl=en',
              '_blank'
            );
            modals.closeCurrent();
          }}
  ...
  return (
    <div className="flex flex-col gap-[16px] pt-[8px]">
      ...
      <ul className="flex flex-col gap-[8px] list-disc ps-[20px] text-[14px] text-textColor/80">
        ...
        <li>
          Hootnshoot does not take responsibility for any issues arising or account
...
export const AddProviderComponent: FC<{
  social: Array<{
    ...
    name: string;
    toolTip?: string;
    category?: 'social' | 'ads' | 'communication';
    isExternal: boolean;
    isWeb3: boolean;
  ...
  const [activeTab, setActiveTab] = React.useState<'social' | 'ads' | 'communication'>('social');
    ...
    (
      ...
      async () => {
        ...
        const gotoIntegration = async (externalUrl?: string) => {
          ...
          // carry the `hootnshoot://` deep link so the backend redirects
          ...
          const params = [
            ...
            onboardingParam,
            isMobile
              ? `redirectUrl=${encodeURIComponent('hootnshoot://integrations')}`
              : '',
          ]
            .filter(Boolean)
            .join('&');
          ...
          if (isMobile) {
            ...
            // app via `hootnshoot://` once OAuth completes.
          ...
          try {
            ...
            toaster.show(
              t(
                ...
                'Hootnshoot browser extension is not installed or not reachable.'
              ),
              'warning'
            );
            return;
  ...
  const TABS = [
    { key: 'social', label: t('tab_social', 'Social') },
    { key: 'ads', label: t('tab_ads', 'Ads') },
    { key: 'communication', label: t('tab_communication', 'Communication') },
  ] as const;

  const filteredSocial = social.filter((item) => {
    if (props.invite && (item.isExternal || item.isWeb3 || item.isChromeExtension || item.customFields)) {
      return false;
    }
    return (item.category ?? 'social') === activeTab;
  });

  const renderItem = (item: typeof social[number]) => (
    <div
      key={item.identifier}
    ...
    >
      <div>
        {item.identifier === 'youtube' || item.identifier === 'youtube-late' ? (
  ...
  return (
    <div className="w-full flex flex-col gap-[16px] rounded-[4px] relative">
      <div className="flex gap-[4px] border-b border-tableBorder">
        {TABS.map((tab) => {
          const count = social.filter(
            (item) => (item.category ?? 'social') === tab.key
          ).length;
          ...
          return (
            <button
              ...
              className={clsx(
                'px-[16px] py-[8px] text-[13px] font-medium border-b-2 -mb-px transition-colors',
                activeTab === tab.key
            ...
            >
              {tab.label}
              <span className="ml-[6px] text-[11px] text-textColor/40">({count})</span>
            </button>
          );
        })}
      </div>
      <div className="flex flex-col">
        ...
        >
          {filteredSocial.length === 0 ? (
            <div className="col-span-5 text-center text-textColor/40 py-[24px] text-[13px]">
              {t('no_providers_in_category', 'No providers in this category')}
            </div>
          ) : (
            filteredSocial.map(renderItem)
          )}
        </div>
      </div>
    </div>
