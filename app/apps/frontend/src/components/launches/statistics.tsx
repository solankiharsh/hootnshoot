...
export const StatisticsModal: FC<{
  ...
  const loadPostTimeline = useCallback(async () => {
    return (await fetch(`/analytics/post-timeline/${postId}`)).json();
  }, [postId, fetch]);
  ...
  const { data: timelineData } = useSWR(`/analytics/post-timeline/${postId}`, loadPostTimeline, {
    revalidateOnFocus: false,
    revalidateOnReconnect: false,
    revalidateIfStale: false,
    revalidateOnMount: true,
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-[24px]">
          ...
          {Array.isArray(timelineData) && timelineData.length > 0 && (
            <div className="flex flex-col gap-[14px]">
              <h3 className="text-[18px] font-[500]">
                {t('post_analytics_timeline', 'Post Analytics Timeline')}
              </h3>
              <div className="h-[280px] bg-newTableHeader border border-newTableBorder rounded-[12px] p-[12px]">
                <ChartMultiLine
                  labels={timelineData.map((row: any) => row.date || row.day || '')}
                  datasets={[
                    {
                      label: 'Impressions',
                      values: timelineData.map((row: any) => Number(row.impressions || 0)),
                    },
                    {
                      label: 'Likes',
                      values: timelineData.map((row: any) => Number(row.likes || 0)),
                    },
                    {
                      label: 'Comments',
                      values: timelineData.map((row: any) => Number(row.comments || 0)),
                    },
                    {
                      label: 'Shares',
                      values: timelineData.map((row: any) => Number(row.shares || 0)),
                    },
                  ]}
                />
              </div>
