...
import { isContentCopProviderSupported } from '@gitroom/frontend/components/compliance/content-cop-platform';
...
const CALENDAR_IDLE_REFRESH_MS = 3_600_000;
const COMPLIANCE_ACTIVE_POLL_MS = 8_000;

function postNeedsCompliancePoll(post: {
  state?: string;
  complianceJob?: { status?: string } | null;
  integration?: { providerIdentifier?: string };
}): boolean {
  const job = post.complianceJob;
  if (job) {
    const s = job.status;
  ...
  return isContentCopProviderSupported(
    post.integration?.providerIdentifier || ''
  );
}

function somePostNeedsCompliancePoll(
  posts:
    | Array<{
        state?: string;
        complianceJob?: { status?: string } | null;
  ...
  return posts.some(postNeedsCompliancePoll);
}
...
export const CalendarWeekProvider: FC<{
  ...
  const {
    ...
    loadData,
    {
      refreshInterval: CALENDAR_IDLE_REFRESH_MS,
  ...
  useEffect(() => {
    ...
    if (!somePostNeedsCompliancePoll(calendarData?.posts)) {
      return;
    }
    const id = window.setInterval(() => {
      void mutateCalendar();
    }, COMPLIANCE_ACTIVE_POLL_MS);
    return () => window.clearInterval(id);
  }, [filters.display, calendarData?.posts, mutateCalendar]);

  useEffect(() => {
    if (filters.display !== 'list') {
      return;
    }
    if (!somePostNeedsCompliancePoll(listData?.posts)) {
      return;
    }
    const id = window.setInterval(() => {
      void mutateList();
    ...
    return () => window.clearInterval(id);
  }, [filters.display, listData?.posts, mutateList]);
