  ...
  return (
    <div className="col-span-full flex flex-col items-center justify-center py-[48px] px-[24px] bg-newTableHeader border border-newTableBorder rounded-[12px]">
      ...
      <p className="text-[15px] text-newTableText text-center mb-[12px]">
        {t(
          'no_analytics_available_for_selected_range',
          'No analytics available for this channel in the selected date range'
        )}
      </p>
      <button
        onClick={onRefresh}
      ...
      >
        ...
        {t('reconnect_or_refresh_channel', 'Reconnect / Refresh Channel')}
      </button>
    </div>
  );
};
...
export const RenderAnalytics: FC<{
  ...
  date: number;
  initialData?: AnalyticsDataItem[];
  disableFetch?: boolean;
}> = (props) => {
  const { integration, date, initialData, disableFetch = false } = props;
  const [loading, setLoading] = useState(!disableFetch);
  const fetch = useFetch();

  // When parent switches disableFetch to true (data ready), clear any stuck loading state.
  useEffect(() => {
    if (disableFetch) setLoading(false);
  }, [disableFetch]);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      return await (await fetch(`/analytics/${integration.id}?date=${date}`)).json();
    } finally {
      setLoading(false);
    }
  }, [integration, date]);
  ...
  const { data } = useSWR(
    disableFetch ? null : `/analytics-${integration?.id}-${date}`,
    load,
    {
    refreshInterval: 0,
    ...
    revalidateOnMount: !disableFetch,
