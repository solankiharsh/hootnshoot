...
export const ThirdPartyListComponent: FC<{
  ...
  const { reload, savedThirdParties } = props;

  const integrationsList = useCallback(async () => {
    const list: { identifier?: string }[] = await (
      await fetch('/third-party/list')
    ).json();
    return list.filter((p) => {
      if (!p.identifier) {
        return true;
      }
      const nid = p.identifier.toLowerCase().replace(/[.\-_]/g, '');
      if (nid === 'reelfarm') {
        return false;
      }
      return true;
  ...
  const builtinAdded = (savedThirdParties ?? []).filter(
    (p) => p.identifier === 'gener8' || p.identifier === 'aurora'
  );
  ...
  return (
    <div className="grid grid-cols-4 gap-[10px] justify-items-center justify-center">
      ...
      {builtinAdded.map((p) => (
        <div
          key={`builtin-${p.identifier}`}
          className="w-full h-full p-[20px] min-h-[100px] text-[14px] bg-newTableHeader rounded-[8px] transition-all text-textColor relative flex flex-col gap-[15px]"
        >
          <div>
            <img
              className="w-[32px] h-[32px]"
