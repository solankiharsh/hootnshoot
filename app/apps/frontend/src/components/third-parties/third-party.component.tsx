...
const BUILTIN_THIRD_PARTY_IDS = new Set(['gener8', 'aurora']);

export const ThirdPartyMenuComponent: FC<{
  reload: () => void;
  tParty: { id: string; identifier: string };
}> = (props) => {
  const { tParty, reload } = props;
  if (BUILTIN_THIRD_PARTY_IDS.has(tParty.identifier.toLowerCase())) {
    return null;
  }
...
export const ThirdPartyComponent = () => {
  ...
  const { data, isLoading, mutate } = useSWR('third-party-sidebar', integrations, {
  ...
  return (
    <>
                ...
                {!isLoading && !data?.length ? (
                  ...
                  data?.map((p: any) => (
                    ...
                    >
                      ...
                      >
                        <ImageWithFallback
                          fallbackSrc={thirdPartyIconSrc(p.identifier)}
      ...
      <div className="bg-newBgColorInner flex-1 flex-col flex p-[20px] gap-[12px]">
        <ThirdPartyListComponent reload={mutate} savedThirdParties={data} />
      </div>
    </>
  );
};
