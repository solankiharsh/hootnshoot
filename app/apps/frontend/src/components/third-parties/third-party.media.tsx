...
import './providers/gener8.provider';
import './providers/aurora.provider';
import { thirdPartyList } from '@gitroom/frontend/components/third-parties/third-party.wrapper';
import { useLaunchStore } from '@gitroom/frontend/components/new-launch/store';
import { useModals } from '@gitroom/frontend/components/layout/new-modal';
import { thirdPartyIconSrc } from '@gitroom/frontend/components/third-parties/third-party.assets';
import { ThirdPartyContext } from '@gitroom/frontend/components/third-parties/third-party.context';
import {
  logThirdPartyClient,
  safeIntegrationRowSnapshot,
} from '@gitroom/frontend/components/third-parties/third-party.client-diagnostics';

export { useThirdParty } from '@gitroom/frontend/components/third-parties/third-party.context';
      ...
      {!thirdParty && (
        <div className="grid grid-cols-4 gap-[10px] justify-items-center justify-center">
          {thirdParties.map((p: any) => (
            <div
              onClick={() => {
                const rowId = p?.id != null ? String(p.id).trim() : '';
                if (!rowId) {
                  logThirdPartyClient(
                    'integrations_tile_blocked',
                    'row_missing_id',
                    { row: safeIntegrationRowSnapshot(p) },
                    'warn'
                  );
                  return;
                }
                setSelectedIntegrationId(rowId);
                setThirdParty(p);
              }}
            ...
            >
              <div>
                <img
                  ...
                  src={thirdPartyIconSrc(p.identifier)}
                  alt=""
                />
              </div>
...
export const ThirdPartyMedia: FC<{
  ...
  const thirdParties = useCallback(async () => {
    ...
    if (dropped.length > 0) {
      logThirdPartyClient(
        'integrations_media_list',
        'rows_dropped_missing_id',
        {
          dropped: dropped.map((r: any) => safeIntegrationRowSnapshot(r)),
    ...
    <>
      <div className="relative group">
        <div
          ...
          onClick={() => {
            logThirdPartyClient(
              'integrations_modal_open',
              'ok',
              {
                mediaRowCount: data.length,
                rows: data.map((r: any) => safeIntegrationRowSnapshot(r)),
              },
              'info'
            );
            modals.openModal({
          ...
          data-tooltip-content={t('integrations', 'Integrations')}
        >
