...
export const MediaBox: FC<{
  ...
  const [filterMode, setFilterMode] = useState<'all' | 'layers' | 'raw'>('all');
  ...
  }, [page, debouncedSearch, filterMode]);
  const { data, mutate, isLoading } = useSWR(
    `get-media-${page}-${debouncedSearch}-${filterMode}`,
    loadMedia
  );
  ...
  const openAsTemplate = useCallback(
    (media: any) => async (e: any) => {
      e.stopPropagation();
      let polotnoJson: unknown = null;
      try {
        const res = await fetch(`/media/${media.id}/polotno-json`);
        const data = await res.json();
        polotnoJson = data?.polotnoJson ?? null;
      } catch (err) {
        console.error('[MediaBox] failed to fetch polotno json:', err);
      }
      if (!polotnoJson) {
        toaster.show(
          t(
            'layers_not_ready',
            'Layers are not ready for this image yet.'
          ),
          'warning'
        );
        return;
      ...
      modals.openModal({
        ...
        children: (close) => (
          <Polonto
            setMedia={(value) => setMedia(value)}
            closeModal={close}
            initialJson={polotnoJson}
          />
        ),
      });
    },
    [fetch, modals, toaster, t, setMedia]
  );
    ...
    <DropFiles disabled={loading} className="flex flex-col flex-1" onDrop={dragAndDrop}>
      <div className="flex flex-col flex-1">
        ...
        <div className="flex gap-[6px] mt-[8px]">
          {(['all', 'layers', 'raw'] as const).map((mode) => (
            <button
              key={mode}
              type="button"
              onClick={() => setFilterMode(mode)}
              className={`h-[30px] px-[12px] rounded-[6px] text-[12px] font-[500] capitalize transition-colors ${
                filterMode === mode
                  ? 'bg-forth text-white'
                  : 'bg-newBgColorInner text-textColor border border-newColColor hover:border-forth'
              }`}
            >
              {mode === 'all' ? t('filter_all', 'All') : mode === 'layers' ? t('filter_layers', 'Layers') : t('filter_raw', 'Raw')}
            </button>
          ))}
        </div>
        ...
        >
          ...
          >
            ...
            {data?.results
              ?.filter((f: any) => {
                if (brokenMediaIds.has(f.id)) return false;
                if (type === 'video') {
              ...
              .map((media: any) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute bottom-[10px] end-[10px] z-[100]">{(media.originalName ?? '').replace(/\.[^.]+$/, '')}</div>
                    {(media as any).decomposeStatus === 'ready' && (
                      <button
                        type="button"
                        onClick={openAsTemplate(media)}
                        title={t('open_as_editable_template', 'Open as editable template')}
                        className="absolute top-[10px] start-[10px] z-[105] cursor-pointer px-[8px] py-[3px] text-[11px] font-[600] rounded-[6px] bg-[#612BD3] text-white shadow"
                      >
                        ✨ {t('layers', 'Layers')}
                      </button>
                    )}
                    {(media as any).decomposeStatus === 'pending' && (
                      <span
                        className="absolute top-[10px] start-[10px] z-[105] px-[8px] py-[3px] text-[11px] font-[500] rounded-[6px] bg-black/50 text-white"
                        title={t('decomposing_layers', 'Decomposing layers…')}
                      >
                        … {t('layers', 'Layers')}
                      </span>
                    )}
                    <div className="w-full h-full rounded-[6px] overflow-hidden relative">
                      ...
                      {media.path.indexOf('mp4') > -1 ? (
                        ...
                        <img
                          ...
                          src={mediaDirectory.set(media.path)}
                          alt="media"
                          onError={() => markMediaBroken(media.id)}
                        />
                      )}
                    </div>
                  </div>
...
export const MultiMediaComponent: FC<{
  ...
  /** Shown between Insert media and Design media (e.g. AI caption generate). */
  captionGenerateAction?: React.ReactNode;
  onChange: (event: {
    target: {
      name: string;
      value?: Array<{
  ...
  const currentMediaRef = useRef(value);
  const mediaDirectory = useMediaDirectory();

  useEffect(() => {
    if (value) {
      setCurrentMedia(value);
      currentMediaRef.current = value;
    }
  }, [value]);
  ...
  const changeMedia = useCallback(
    (
      ...
      const prev = currentMediaRef.current;
      const newMedia = [...(prev || []), ...mediaArray];
  ...
  const replaceMedia = useCallback(
    (
      ...
      const mediaArray = Array.isArray(m) ? m : [m];
      currentMediaRef.current = mediaArray;
      ...
      window.dispatchEvent(new CustomEvent('media:design-saved'));
    },
    [name, onChange]
  );

  const designMedia = useCallback(async () => {
    if (!user?.tier?.ai || dummy) return;

    const attached = currentMediaRef.current?.[0];
    let initialJson: unknown = null;
    if (attached?.id) {
      try {
        const res = await fetch(`/media/${attached.id}/polotno-json`);
        if (res.ok) {
          const data = await res.json();
          if (data?.polotnoJson) {
            initialJson = data.polotnoJson;
          } else if (data?.decomposeStatus === 'pending') {
            toaster.show(
              t(
                'layers_decomposing',
                'Layers are still being decomposed — opening blank canvas. Try again in a few seconds.'
              ),
              'warning'
            );
          }
        ...
        console.error('[MultiMediaComponent] failed to fetch polotno json:', err);
      }
    }
  ...
  }, [replaceMedia, fetch, modals, t, toaster, user?.tier?.ai, dummy]);
