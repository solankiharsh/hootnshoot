...
const nextConfig = {
  experimental: {
    // Gener8 image jobs can run several minutes (/image/generate, /third-party/.../submit).
  ...
  transpilePackages: ['crypto-hash', 'openpolotno'],
    ...
    return [
      {
        ...
        destination:
          storageProvider === 'local' ? '/uploads/:path*' : '/404',
        permanent: true,
      },
    ];
  },
    ...
    return [
      {
        source: '/uploads/:path*',
        destination:
          storageProvider === 'local' ? '/api/uploads/:path*' : '/404',
      },
    ];
  },
};
