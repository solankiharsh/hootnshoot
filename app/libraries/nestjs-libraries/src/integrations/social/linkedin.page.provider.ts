  ...
  implements SocialProvider
{
  ...
  override async refreshToken(
    ...
    const credentials = this.getPageAppCredentials();
    const {
    ...
    const { memberId, pages } = await this.loadAdminPages(accessToken);
    ...
    return {
      ...
      name: primaryPage?.name || 'LinkedIn Page',
      picture: primaryPage?.picture || '',
  ...
  async companies(accessToken: string) {
    return (await this.loadAdminPages(accessToken)).pages;
  }
  ...
  private getPageAppCredentials() {
    const clientId = process.env.LINKEDIN_PAGE_CLIENT_ID;
    const clientSecret = process.env.LINKEDIN_PAGE_CLIENT_SECRET;

    if (!clientId || !clientSecret) {
      throw new Error(
        'Missing LinkedIn Page app credentials. Set LINKEDIN_PAGE_CLIENT_ID and LINKEDIN_PAGE_CLIENT_SECRET.'
      );
    }
  ...
  private async loadAdminPages(accessToken: string) {
    const { elements } = await (
      await fetch(
        'https://api.linkedin.com/v2/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&projection=(elements*(roleAssignee,organizationalTarget,organizationalTarget~(localizedName,vanityName,logoV2(original~:playableStreams))))',
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
    ...
    if (!pages.length) {
      throw new Error(
        'No LinkedIn Pages were found for this account. Make sure your user is an ADMINISTRATOR of the target page.'
      );
    }

    const memberId =
      elements?.[0]?.roleAssignee?.split?.(':')?.pop?.() ||
      `linkedin-page-admin-${pages[0].id}`;

    return { memberId, pages };
  }
