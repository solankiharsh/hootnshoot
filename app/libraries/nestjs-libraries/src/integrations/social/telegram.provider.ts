import {
  AuthTokenDetails,
  PostDetails,
  PostResponse,
  SocialProvider,
} from '@gitroom/nestjs-libraries/integrations/social/social.integrations.interface';
import { makeId } from '@gitroom/nestjs-libraries/services/make.is';
import dayjs from 'dayjs';
import { SocialAbstract } from '@gitroom/nestjs-libraries/integrations/social.abstract';
//@ts-ignore
import mime from 'mime';
import TelegramBot from 'node-telegram-bot-api';
import { Integration } from '@prisma/client';
import striptags from 'striptags';
import { createReadStream } from 'node:fs';
import { access, constants as fsConstants } from 'node:fs/promises';
import { Readable } from 'node:stream';

const telegramBot = new TelegramBot(process.env.TELEGRAM_TOKEN!);
// Added to support local storage posting
const frontendURL = process.env.FRONTEND_URL || 'http://localhost:5000';

/** https://core.telegram.org/bots/api — captions on media are shorter than plain messages (4096). */
const TELEGRAM_MEDIA_CAPTION_MAX = 1024;

export class TelegramProvider extends SocialAbstract implements SocialProvider {
  override maxConcurrentJob = 3; // Telegram has moderate bot API limits
  identifier = 'telegram';
  name = 'Telegram';
  override category = 'communication' as const;
  isBetweenSteps = false;
  isWeb3 = true;
  scopes = [] as string[];
  editor = 'html' as const;
  maxLength() {
    return 4096;
  }

  async refreshToken(refresh_token: string): Promise<AuthTokenDetails> {
    return {
      refreshToken: '',
      expiresIn: 0,
      accessToken: '',
      id: '',
      name: '',
      picture: '',
      username: '',
    };
  }

  async generateAuthUrl() {
    const state = makeId(17);
    return {
      url: state,
      codeVerifier: makeId(10),
      state,
    };
  }

  async authenticate(params: {
    code: string;
    codeVerifier: string;
    refresh?: string;
  }) {
    const chat = await telegramBot.getChat(params.code);

    console.log(JSON.stringify(chat));
    if (!chat?.id) {
      return 'No chat found';
    }

    const photo = !chat?.photo?.big_file_id
      ? ''
      : await telegramBot.getFileLink(chat.photo.big_file_id);

    // Modified id to work with chat.username (public groups/channels) or chat.id (private groups/channels) when chat.username is not available
    return {
      id: String(chat.username ? chat.username : chat.id),
      name: chat.title!,
      accessToken: String(chat.id),
      refreshToken: '',
      expiresIn: dayjs().add(200, 'year').unix() - dayjs().unix(),
      picture: photo || '',
      username: chat.username!,
    };
  }

  async getBotId(query: { id?: number; word: string }) {
    // Added allowed_updates Ensure only necessary updates are fetched
    const res = await telegramBot.getUpdates({
      ...(query.id ? { offset: query.id } : {}),
      allowed_updates: ['message', 'channel_post'],
    });
    //message.text is for groups, channel_post.text is for channels
    const match = res.find(
      (p) =>
        (p?.message?.text === `/connect ${query.word}` &&
          p?.message?.chat?.id) ||
        (p?.channel_post?.text === `/connect ${query.word}` &&
          p?.channel_post?.chat?.id)
    );
    // get correct chatId based on the channel type
    const chatId = match?.message?.chat?.id || match?.channel_post?.chat?.id;

    // prevents the code from running while chatId is still undefined to avoid the error 'ETELEGRAM: 400 Bad Request: chat_id is empty'. the code would still work eventually but console spam is not pretty
    if (chatId) {
      //get the numberic ID of the bot
      const botId = (await telegramBot.getMe()).id;
      // check if the bot is an admin in the chat
      const isAdmin = await this.botIsAdmin(chatId, botId);
      // get the messageId of the message that triggered the connection
      const connectMessageId =
        match?.message?.message_id || match?.channel_post?.message_id;

      if (!isAdmin) {
        // alternatively you can replace this with a console.log if you do not want to inform the user of the bot's admin status
        telegramBot.sendMessage(
          chatId,
          "Connection Successful. I don't have admin privileges to delete these messages, please go ahead and remove them yourself."
        );
      } else {
        // Delete the message that triggered the connection
        await telegramBot.deleteMessage(chatId, connectMessageId);
        // Send success message to the chat
        const successMessage = await telegramBot.sendMessage(
          chatId,
          'Connection Successful. Message will be deleted in 10 seconds.'
        );
        // Delete the success message after 10 seconds
        setTimeout(async () => {
          await telegramBot.deleteMessage(chatId, successMessage.message_id);
          console.log('Success message deleted.');
        }, 10000);
      }
    }

    // modified lastChatId to work with any type of channel (private/public groups/channels)
    return chatId
      ? { chatId }
      : res.length > 0
      ? {
          lastChatId: res[res.length - 1].update_id + 1,
        }
      : {};
  }

  /** Public URL used only when the file is not readable on local disk (e.g. orchestrator has no UPLOAD_DIRECTORY copy). */
  private telegramFetchableMediaUrl(
    media: { path: string; url?: string }
  ): string {
    const tryUrl = (s: string | undefined): string | undefined => {
      if (!s?.trim()) {
        return undefined;
      }
      const t = s.trim();
      if (!/^https?:\/\//i.test(t)) {
        return undefined;
      }
      try {
        return new URL(t).hostname ? t : undefined;
      } catch {
        return undefined;
      }
    };

    const fromUrl = tryUrl(media.url);
    if (fromUrl) {
      return fromUrl;
    }
    const fromPath = tryUrl(media.path);
    if (fromPath) {
      return fromPath;
    }

    const base = (process.env.FRONTEND_URL || frontendURL).replace(/\/$/, '');
    if (!base || !/^https?:\/\//i.test(base)) {
      throw new Error(
        'Telegram: media is not a public URL and FRONTEND_URL is missing or invalid; set FRONTEND_URL where the orchestrator runs'
      );
    }
    const relPath = media.path.indexOf('http') === -1 ? media.path : '';
    if (!relPath?.trim()) {
      throw new Error('Telegram: empty media path');
    }
    const built =
      base +
      '/' +
      (process.env.NEXT_PUBLIC_UPLOAD_STATIC_DIRECTORY || '') +
      relPath;
    try {
      const u = new URL(built.replace(/([^:]\/)\/+/g, '$1'));
      if (!u.hostname) {
        throw new Error('empty host');
      }
      return built.replace(/([^:]\/)\/+/g, '$1');
    } catch {
      throw new Error(
        'Telegram: could not build a fetchable media URL; check FRONTEND_URL and NEXT_PUBLIC_UPLOAD_STATIC_DIRECTORY'
      );
    }
  }

  private filenameFromMedia(media: { path: string; url?: string }): string {
    const fromPath = media.path?.split('/').filter(Boolean).pop();
    if (fromPath) {
      return fromPath;
    }
    try {
      return (
        new URL(media.url || '')
          .pathname.split('/')
          .filter(Boolean)
          .pop() || 'attachment'
      );
    } catch {
      return 'attachment';
    }
  }

  /**
   * Upload file bytes/streams to Telegram (multipart), not remote HTTP URLs.
   * Prefer `media.path` when it is a readable path on this host (see `updateMedia`).
   */
  private async resolveTelegramMediaInput(media: {
    path: string;
    url?: string;
  }): Promise<{ body: Buffer | Readable; contentType: string }> {
    const diskPath = media.path?.trim();
    if (diskPath && !/^https?:\/\//i.test(diskPath)) {
      try {
        await access(diskPath, fsConstants.R_OK);
        const contentType =
          mime.getType(diskPath) || 'application/octet-stream';
        return { body: createReadStream(diskPath), contentType };
      } catch {
        /* fall through to fetch */
      }
    }

    const fetchUrl = this.telegramFetchableMediaUrl(media);
    const res = await fetch(fetchUrl);
    if (!res.ok) {
      throw new Error(
        `Telegram: could not read media file or download it (${res.status})`
      );
    }
    const contentType =
      res.headers.get('content-type')?.split(';')[0]?.trim() ||
      mime.getType(fetchUrl) ||
      'application/octet-stream';

    if (res.body != null && typeof Readable.fromWeb === 'function') {
      return {
        body: Readable.fromWeb(
          res.body as import('stream/web').ReadableStream<Uint8Array>
        ),
        contentType,
      };
    }

    return {
      body: Buffer.from(await res.arrayBuffer()),
      contentType,
    };
  }

  private async processMedia(mediaFiles: PostDetails['media']) {
    const list = mediaFiles || [];
    const out: {
      type: 'photo' | 'video' | 'document';
      media: Buffer | Readable;
      fileOptions: { filename: string; contentType: string };
    }[] = [];

    for (const raw of list) {
      const media = raw as { path: string; url?: string };
      const { body, contentType } = await this.resolveTelegramMediaInput(media);
      let mediaType: 'photo' | 'video' | 'document';
      if (contentType.startsWith('image/')) {
        mediaType = 'photo';
      } else if (contentType.startsWith('video/')) {
        mediaType = 'video';
      } else {
        mediaType = 'document';
      }
      out.push({
        type: mediaType,
        media: body,
        fileOptions: {
          filename: this.filenameFromMedia(media),
          contentType,
        },
      });
    }
    return out;
  }

  private captionForMedia(html: string): {
    caption: string;
    parse_mode?: 'HTML';
  } {
    if (html.length <= TELEGRAM_MEDIA_CAPTION_MAX) {
      return { caption: html, parse_mode: 'HTML' };
    }
    const plain = striptags(html);
    if (plain.length <= TELEGRAM_MEDIA_CAPTION_MAX) {
      return { caption: plain };
    }
    const ellipsis = '…';
    const max = TELEGRAM_MEDIA_CAPTION_MAX - ellipsis.length;
    return { caption: plain.slice(0, max) + ellipsis };
  }

  private async sendMessage(
    accessToken: string,
    message: PostDetails,
    replyToMessageId?: number
  ): Promise<number | null> {
    let messageId: number | null = null;
    const mediaFiles = message.media || [];
    const raw = message.message || '';
    const withoutParagraphs = raw
      .replace(/<\/p>\s*/gi, '\n')
      .replace(/<p[^>]*>/gi, '');
    const text = striptags(withoutParagraphs, ['u', 'strong'])
      .replace(/<strong>/g, '<b>')
      .replace(/<\/strong>/g, '</b>');

    console.log(text);
    const processedMedia = await this.processMedia(mediaFiles);

    // if there's no media, bot sends a text message only
    if (processedMedia.length === 0) {
      const response = await telegramBot.sendMessage(accessToken, text, {
        parse_mode: 'HTML',
        ...(replyToMessageId ? { reply_to_message_id: replyToMessageId } : {}),
      });
      messageId = response.message_id;
    }
    // if there's only one media, bot sends the media with the text message as caption
    else if (processedMedia.length === 1) {
      const media = processedMedia[0];
      const cap = this.captionForMedia(text);
      const options = {
        caption: cap.caption,
        ...(cap.parse_mode ? { parse_mode: cap.parse_mode as const } : {}),
        ...(replyToMessageId ? { reply_to_message_id: replyToMessageId } : {}),
      };
      const response =
        media.type === 'video'
          ? await telegramBot.sendVideo(
              accessToken,
              media.media,
              options,
              media.fileOptions
            )
          : media.type === 'photo'
          ? await telegramBot.sendPhoto(
              accessToken,
              media.media,
              options,
              media.fileOptions
            )
          : await telegramBot.sendDocument(
              accessToken,
              media.media,
              options,
              media.fileOptions
            );
      messageId = response.message_id;
    }
    // if there are multiple media, bot sends them as a media group - max 10 media per group - with the text as a caption (if there are more than 1 group, the caption will only be sent with the first group)
    else {
      const cap = this.captionForMedia(text);
      const mediaGroups = this.chunkMedia(processedMedia, 10);
      for (let i = 0; i < mediaGroups.length; i++) {
        const mediaGroup = mediaGroups[i].map((m, index) => ({
          type: m.type === 'document' ? 'document' : m.type, // Documents are not allowed in media groups
          media: m.media,
          ...(i === 0 && index === 0
            ? {
                caption: cap.caption,
                ...(cap.parse_mode ? { parse_mode: cap.parse_mode } : {}),
              }
            : {}),
        }));

        const response = await telegramBot.sendMediaGroup(
          accessToken,
          mediaGroup as any[],
          {
            ...(replyToMessageId && i === 0
              ? { reply_to_message_id: replyToMessageId }
              : {}),
          }
        );
        if (i === 0) {
          messageId = response[0].message_id;
        }
      }
    }

    return messageId;
  }

  async post(
    id: string,
    accessToken: string,
    postDetails: PostDetails[]
  ): Promise<PostResponse[]> {
    const [firstPost] = postDetails;

    const messageId = await this.sendMessage(accessToken, firstPost);

    // for private groups/channels message.id is undefined so the link generated by Hootnshoot will be unusable "https://t.me/c/undefined/16"
    // to avoid that, we use accessToken instead of message.id and we generate the link manually removing the -100 from the start.
    if (messageId) {
      return [
        {
          id: firstPost.id,
          postId: String(messageId),
          releaseURL: `https://t.me/${
            id !== 'undefined' ? id : `c/${accessToken.replace('-100', '')}`
          }/${messageId}`,
          status: 'completed',
        },
      ];
    }

    return [];
  }

  async comment(
    id: string,
    postId: string,
    lastCommentId: string | undefined,
    accessToken: string,
    postDetails: PostDetails[],
    integration: Integration
  ): Promise<PostResponse[]> {
    const [commentPost] = postDetails;
    const replyToId = Number(lastCommentId || postId);

    const messageId = await this.sendMessage(accessToken, commentPost, replyToId);

    if (messageId) {
      return [
        {
          id: commentPost.id,
          postId: String(messageId),
          releaseURL: `https://t.me/${
            id !== 'undefined' ? id : `c/${accessToken.replace('-100', '')}`
          }/${messageId}`,
          status: 'completed',
        },
      ];
    }

    return [];
  }
  // chunkMedia is used to split media into groups of "size". 10 is used here because telegram api allows a maximum of 10 media per group
  private chunkMedia(
    media: { type: string; media: Buffer | Readable }[],
    size: number
  ) {
    const result = [];
    for (let i = 0; i < media.length; i += size) {
      result.push(media.slice(i, i + size));
    }
    return result;
  }

  async botIsAdmin(chatId: number, botId: number): Promise<boolean> {
    try {
      const chatMember = await telegramBot.getChatMember(chatId, botId);

      if (
        chatMember.status === 'administrator' ||
        chatMember.status === 'creator'
      ) {
        const permissions = chatMember.can_delete_messages;
        return !!permissions; // Return true if bot can delete messages
      }

      return false;
    } catch (error) {
      console.error('Error checking bot privileges:', error);
      return false;
    }
  }
}
