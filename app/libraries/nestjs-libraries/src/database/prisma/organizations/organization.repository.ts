...
export class OrganizationRepository {
            ...
            user: {
              create: {
                ...
                email: email
                  ...
                  : `${saasName}+` + makeId(10) + '@hootnshoot.local',
                name: name ? `${name}###${id}` : `Unnamed User###${id}`,
                providerName: 'LOCAL',
  ...
  async createOrgAndUser(
    ...
    const orgName =
      (body.company && String(body.company).trim()) || 'My Organization';

    const sharedOrg = await this._organization.model.organization.findFirst({
      where: { name: orgName },
      select: { id: true },
  ...
  async createPersonalOrg(userId: string, displayName: string) {
    return this._organization.model.organization.create({
      data: {
        ...
        isTrailing: true,
        users: {
          create: { userId, role: Role.SUPERADMIN },
        },
      },
      select: { id: true },
    });
  ...
  async createNamedOrg(userId: string, companyName: string) {
    ...
    if (existing) {
      const alreadyMember = await this._userOrg.model.userOrganization.findUnique({
        where: { userId_organizationId: { userId, organizationId: existing.id } },
      });
      if (!alreadyMember) {
        await this._userOrg.model.userOrganization.create({
          data: { userId, organizationId: existing.id, role: Role.SUPERADMIN },
        });
      }
      return existing;
    }
    return this._organization.model.organization.create({
      data: {
        ...
        allowTrial: true,
        isTrailing: true,
        users: { create: { userId, role: Role.SUPERADMIN } },
      },
      select: { id: true },
    });
  }
