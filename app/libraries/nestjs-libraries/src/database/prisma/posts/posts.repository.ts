...
function workflowErrorForPost(err: unknown): string {
  ...
  try {
    ...
    return s !== '{}' ? s : 'An error occurred while publishing this post';
  } catch {
    return 'An error occurred while publishing this post';
  }
}
...
export class PostsRepository {
  ...
  async changeState(id: string, state: State, err?: any, body?: any) {
    const update = await this._post.model.post.update({
      ...
      data: {
        state,
        ...(err ? { error: workflowErrorForPost(err) } : {}),
      },
      include: {
        integration: {
          select: {
    ...
    if (state === 'ERROR' && err && body) {
      try {
        await this._errors.model.errors.create({
          data: {
            message: workflowErrorForPost(err),
