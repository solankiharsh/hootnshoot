...
export class PostsService {
  ...
  private async _attachLatestComplianceJobs<
    T extends { group?: string | null },
  >(orgId: string, posts: T[]): Promise<Array<T & { complianceJob: unknown }>> {
    if (!isComplianceEnabled()) {
      return posts as Array<T & { complianceJob: unknown }>;
    }
    if (!posts?.length) {
      return posts as Array<T & { complianceJob: unknown }>;
    }
    const groups = [
      ...new Set(
        posts.map((p) => p.group).filter((g): g is string => typeof g === 'string' && !!g)
      ),
    ];
    if (!groups.length) {
      return posts.map((p) => ({ ...p, complianceJob: null }));
    }
    const jobs = await this._prisma.complianceJob.findMany({
      where: { orgId, postGroupId: { in: groups } },
    ...
    const byGroup = new Map<string, (typeof jobs)[0]>();
    for (const j of jobs) {
      if (!byGroup.has(j.postGroupId)) {
        byGroup.set(j.postGroupId, j);
      }
    }
    return posts.map((p) => ({
      ...p,
      complianceJob: (p.group && byGroup.get(p.group)) || null,
    }));
  }
  ...
  async checkPostAnalyticsTimeline(
    orgId: string,
    postId: string,
    forceRefresh = false
  ): Promise<any[]> {
    const post = await this._postRepository.getPostById(postId, orgId);
    if (!post || !post.releaseId) return [];
    ...
    try {
      ...
      if (e instanceof RefreshToken) {
        return this.checkPostAnalyticsTimeline(orgId, postId, true);
      }
      return [];
    }
  }
  ...
  async getPostsMinified(orgId: string, query: GetPostsDto) {
    ...
    const withJobs = await this._attachLatestComplianceJobs(orgId, posts);
    return minifyPosts({
      posts: withJobs,
    });
  }
  ...
  async getPostsList(orgId: string, query: GetPostsListDto) {
    const result = await this._postRepository.getPostsList(orgId, query);
    const posts = await this._attachLatestComplianceJobs(orgId, result.posts);
      ...
      const getImageList = await Promise.all(
        (
          ...
          .map((m) => {
            ...
            const isVideo = videoExtensions.some((ext) => m.path.toLowerCase().includes(ext));
            return {
              ...m,
              url:
  ...
  async getPostsByGroup(orgId: string, group: string) {
    ...
    const complianceJob = await this._prisma.complianceJob.findFirst({
      where: { orgId, postGroupId: group },
      orderBy: { createdAt: 'desc' },
      select: {
        id: true,
        status: true,
  ...
  async startWorkflow(
    ...
    logger.info('startWorkflow: starting Temporal workflow', {
      event: 'hootnshoot.post.startWorkflow.start',
      postId,
      orgId,
      taskQueue,
    });
    ...
    try {
      ...
      logger.info('startWorkflow: Temporal workflow started', {
        event: 'hootnshoot.post.startWorkflow.done',
        postId,
        orgId,
        taskQueue,
        workflowId: `post_${postId}`,
      ...
      logger.error('startWorkflow: failed to start Temporal workflow', {
        event: 'hootnshoot.post.startWorkflow.error',
        postId,
        orgId,
        taskQueue,
  ...
  async createPost(orgId: string, body: CreatePostDto): Promise<any[]> {
    logger.info('createPost started', {
      event: 'hootnshoot.post.createPost.start',
      orgId,
      type: body.type,
      date: body.date,
      postCount: body.posts?.length,
      providers: body.posts?.map((p) => (p.settings as any)?.__type),
    });
    const postList = [];
    for (const post of body.posts) {
      ...
      if (body.type !== 'update' && !body.publishWhenApproved) {
        this.startWorkflow(
  ...
  /**
   * Publish a post group immediately after Content Cop approval (Post now flow).
   * Post stays on the calendar if not compliant.
   */
  async publishNowForApprovedGroup(
    orgId: string,
    postGroupId: string
  ): Promise<void> {
    const gate = await this.gatePublishByComplianceOrHold(orgId, postGroupId);
    if (!gate.ok) {
      return;
    }
    const now = new Date();
    await this._prisma.post.updateMany({
      where: {
        ...
        group: postGroupId,
        deletedAt: null,
        state: { in: ['QUEUE', 'HELD_COMPLIANCE'] },
      },
      data: { publishDate: now, state: 'QUEUE', error: null },
    });
    const root = await this._prisma.post.findFirst({
      where: {
        organizationId: orgId,
        group: postGroupId,
        parentPostId: null,
  ...
  /**
   * When compliance is satisfied, move posts from HELD_COMPLIANCE to QUEUE and restart the publish workflow.
   */
  async releaseComplianceHoldAndRestartWorkflow(
    orgId: string,
    postGroupId: string
  ): Promise<void> {
    const held = await this._prisma.post.count({
      where: {
        organizationId: orgId,
        group: postGroupId,
  ...
  async gatePublishByComplianceOrHold(
    ...
    const latestJob = await this._prisma.complianceJob.findFirst({
      where: { orgId, postGroupId },
      orderBy: { createdAt: 'desc' },
    });
    const compliant =
      !!latestJob &&
      (latestJob.overriddenAt != null ||
        (latestJob.status === 'completed' &&
          latestJob.decision === 'Approved'));
    if (compliant) {
      return { ok: true };
    }
    const reason = !latestJob
      ? 'compliance_pending_no_job'
      : latestJob.decision === 'Rejected'
        ? 'compliance_rejected'
