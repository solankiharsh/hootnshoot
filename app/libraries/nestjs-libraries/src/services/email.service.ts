...
export class EmailService {
  ...
  async sendEmail(
    ...
    const signal = this._temporalService.client
      .getRawClient()
    ...
    // Race the Temporal call against a timeout so a slow/unready Temporal
    // instance (e.g. during local startup) never blocks the caller.
    // On prod Temporal responds in milliseconds so this has no impact there.
    const timeout = new Promise<void>((_, reject) =>
      setTimeout(
        () => reject(new Error('Temporal signalWithStart timed out')),
        TEMPORAL_TIMEOUT_MS
      )
    );
    ...
    try {
      ...
      console.error('sendEmail: failed to signal Temporal workflow:', err);
    }
  }
