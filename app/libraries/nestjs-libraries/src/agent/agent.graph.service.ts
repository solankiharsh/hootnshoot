...
export class AgentGraphService {
  ...
  async generatePictures(state: WorkflowChannelsState) {
    ...
    const gener8Available = !!process.env.GENER8_API_KEY;
        ...
        if (gener8Available) {
          try {
            const result = await this._imageService.generate({
              inputText: p.prompt || p.content,
              personas: state.imagePersona ? [state.imagePersona] : undefined,
              onBrand: true,
            });
            // Gener8 returns image URLs; use the first one
            return { ...p, image: result.images[0] };
          } catch (err) {
            console.error('[AgentGraphService] Gener8 failed, falling back to DALL-E:', err);
          }
        }
        // DALL-E fallback
    ...
    return app.streamEvents(
      {
        ...
        imagePersona: body.imagePersona,
        format: body.format,
        tone: body.tone,
        orgId,
      },
