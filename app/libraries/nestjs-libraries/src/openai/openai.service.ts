...
export class OpenaiService {
  async translateText(text: string, targetLanguageName: string): Promise<string> {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a professional translator. Translate the user's text into ${targetLanguageName}. Return ONLY the translated text with no explanation or extra commentary.`,
        },
  ...
  async generateImage(prompt: string, _isUrl?: boolean, isVertical = false) {
    ...
    if (!b64) {
      throw new HttpException('Image generation returned no image data', 502);
    }
    return b64;
  }
