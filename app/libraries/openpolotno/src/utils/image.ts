...
/** Active page dimensions (not store defaults, which may differ from the canvas page). */
export function getActivePageDimensions(store: StoreType): {
  width: number;
  height: number;
} {
  const page = store.activePage;
  return {
    width: page?.computedWidth ?? store.width,
    height: page?.computedHeight ?? store.height,
  };
}

/** Scale element to fit inside the active page and center it (or place at drop position). */
export function fitAndCenterOnPage(
  ...
  const { width: pageW, height: pageH } = getActivePageDimensions(store);
  const scaleFactor = options?.scaleFactor ?? 1;
  const maxScale = options?.maxScale ?? 1;

  let width = naturalWidth || 1;
  let height = naturalHeight || 1;

  const ratio =
    scaleFactor * Math.min(pageW / width, pageH / height, maxScale);
  width *= ratio;
  height *= ratio;

  const x = (droppedPos?.x ?? pageW / 2) - width / 2;
  const y = (droppedPos?.y ?? pageH / 2) - height / 2;
